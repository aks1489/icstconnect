...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
    ...
    const checkUserExists = async (email: string): Promise<boolean> => {
        // Try to fetch public profile info to check existence
        try {
            ...
            if (error) {
                return true // Assume exists if we can't check due to RLS
            }
            return !!data
        } catch {
            return true
            ...
            if (isForgotPassword) {
                ...
                if (error) {
                    if (error.message === 'Invalid login credentials') {
                        ...
                        if (!exists) {
                            throw new Error('Email is not registered, sign up now!')
                        }
                    }
                    throw error
                }
            ...
            <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-0 overflow-hidden animate-in fade-in zoom-in duration-300 mx-4">
                ...
                <div className="p-8 pt-6">
                    <AuthTabs
                        ...
                        onTabChange={(login) => { setIsLogin(login); setIsForgotPassword(false); }}
                    />
...
const AuthHeader = ({ isLogin, onClose }: { isLogin: boolean, onClose: () => void }) => (
...
const AuthInputs = ({ isLogin, email, setEmail, password, setPassword, fullName, setFullName, onForgotPassword }: any) => (
    <>
        {!isLogin && (
            <div className="space-y-1.5 animate-in fade-in slide-in-from-right-4 duration-300">
...
const SubmitButton = ({ loading, isLogin, isForgotPassword, showOtpInput }: any) => (
    <button
        type="submit"
        disabled={loading}
